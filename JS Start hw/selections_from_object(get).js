...
for (const user of users) {
  //проходим по каждому элементу(объекту) в массиве
...
for (const user of users) {
  //проходим по каждому элементу(объекту) в массиве
  if (user.friends.length > lengthFriends) {
    lengthFriends = user.friends.length;
  }
}
...
for (const user of users) {
  //проходим по каждому элементу(объекту) в массиве
  if (user.friends.length === lengthFriends) {
    let thisNameFirst = user.name.first;
    let thisNameLast = user.name.last;

    console.log(
      `${thisNameFirst} ${thisNameLast} has ${lengthFriends} friends. And this are: `
    );

    for (const friend of user.friends) {
      //проходим по каждому элементу(объекту) в массиве user.friends

      let namesFriends = [];
      namesFriends.push(friend.name);
...
for (const object of users) {
  const balanceCounter = parseFloat(object.balance);

  console.log(balanceCounter);

  if (maxBalance === null || balanceCounter > maxBalance) {
    maxBalance = balanceCounter;
    isCompanyMax = object.company;
  }
  if (minBalance === null || balanceCounter < minBalance) {
    minBalance = balanceCounter;
    isCompanyMin = object.company;
  }
}
console.log(`max balance - ${maxBalance} for this company [${isCompanyMax}].`);
console.log(`min balance - ${minBalance} for this company [${isCompanyMin}].`);